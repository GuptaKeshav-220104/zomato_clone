import React from 'react'
import { FaDribbbleSquare,FaFacebookSquare,FaGithubSquare,FaInstagram,FaTwitterSquare } from 'react-icons/fa'

const Footer = () => {
  return (
    <div className='max-w-[1520px] m-auto px-4 py-2 bg-[#24262b]'>
        <div className='py-16 px-4 grid lg:grid-cols-3 gap-8 text-gray-300'>
            <div>
            <h1 className='w-full text-3xl font-bold text-orange-500 '>YumEats</h1>
            <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatum
            molestiae delectus culpa hic assumenda, voluptate reprehenderit
            dolore autem cum ullam sed odit perspiciatis. 
            </p>
            <div className='flex justify-between md:w-[75%] my-6'>
                <FaFacebookSquare size={30}/>
                <FaInstagram size={30}/>
                <FaTwitterSquare size={30}/>
                <FaGithubSquare size={30}/>
                <FaDribbbleSquare size={30}/>
            </div>
        </div>
        <div className='lg:col-span-2 flex justify-between mt-6'>
            <div>
                <h6 className='font-medium text-[#9b9b9b]'>Location</h6>
                <ul>
                    <li className='py-2 text-sm'>Delhi</li>
                    <li className='py-2 text-sm'>Bombay</li>
                    <li className='py-2 text-sm'>Jaipur</li>
                    <li className='py-2 text-sm'>Hyderabad</li>
                </ul>
            </div>
            <div>
                <h6 className='font-medium text-[#9b9b9b]'>Location</h6>
                <ul>
                    <li className='py-2 text-sm'>Benglore</li>
                    <li className='py-2 text-sm'>Ranchi</li>
                    <li className='py-2 text-sm'>Chennai</li>
                    <li className='py-2 text-sm'>Pune</li>
                </ul>
            </div>
            <div>
                <h6 className='font-medium text-[#9b9b9b]'>Location</h6>
                <ul>
                    <li className='py-2 text-sm'>Kolkata</li>
                    <li className='py-2 text-sm'>Madras</li>
                    <li className='py-2 text-sm'>Jharkhand</li>
                    <li className='py-2 text-sm'>Allahabad</li>
                </ul>
            </div>
            <div>
                <h6 className='font-medium text-[#9b9b9b]'>Location</h6>
                <ul>
                    <li className='py-2 text-sm'>Bhopal</li>
                    <li className='py-2 text-sm'>Nagpur</li>
                    <li className='py-2 text-sm'>Haryana</li>
                    <li className='py-2 text-sm'>Hamirpur</li>
                </ul>
            </div>
        </div>
        </div>
    </div>
  )
}

export default Footer
